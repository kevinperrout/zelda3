...
static inline __attribute__((always_inline)) Ppu* ppu_init() {
  Ppu* ppu = (Ppu * )malloc(sizeof(Ppu));
  ppu->extraLeftRight = kPpuExtraLeftRight;
  return ppu;
}

static inline __attribute__((always_inline)) void ppu_free(Ppu* ppu) {
  free(ppu);
}

static inline __attribute__((always_inline)) void ppu_reset(Ppu* ppu) {
...
static inline __attribute__((always_inline)) void ppu_saveload(Ppu *ppu, SaveLoadFunc *func, void *ctx) {
  uint8 tmp[556] = { 0 };
...
static inline __attribute__((always_inline)) int PpuGetCurrentRenderScale(Ppu *ppu, uint32_t render_flags) {
...
static inline __attribute__((always_inline)) void PpuBeginDrawing(Ppu *ppu, uint8_t *pixels, size_t pitch, uint32_t render_flags) {
...
static inline __attribute__((always_inline)) void PpuWindows_Clear(PpuWindows *win, Ppu *ppu, uint layer) {
...
static inline __attribute__((always_inline)) void PpuWindows_Calc(PpuWindows *win, Ppu *ppu, uint layer) {
...
static inline __attribute__((always_inline)) void PpuDrawBackground_4bpp(Ppu *ppu, uint y, bool sub, uint layer, PpuZbufType zhi, PpuZbufType zlo) {
...
static inline __attribute__((always_inline)) void PpuDrawBackground_2bpp(Ppu *ppu, uint y, bool sub, uint layer, PpuZbufType zhi, PpuZbufType zlo) {
#define DO_PIXEL(i) do { \
...
static inline __attribute__((always_inline)) void PpuDrawBackground_4bpp_mosaic(Ppu *ppu, uint y, bool sub, uint layer, PpuZbufType zhi, PpuZbufType zlo) {
...
static inline __attribute__((always_inline)) void PpuDrawBackground_2bpp_mosaic(Ppu *ppu, int y, bool sub, uint layer, PpuZbufType zhi, PpuZbufType zlo) {
...
static inline __attribute__((always_inline)) void PpuDrawSprites(Ppu *ppu, uint y, uint sub, bool clear_backdrop) {
  int layer = 4;
  if (!IS_SCREEN_ENABLED(ppu, sub, layer))
    return;  // layer is completely hidden
  PpuWindows win;
...
static inline __attribute__((always_inline)) void PpuDrawBackground_mode7(Ppu *ppu, uint y, bool sub, PpuZbufType z) {
  int layer = 0;
  if (!IS_SCREEN_ENABLED(ppu, sub, layer))
    return;  // layer is completely hidden
  PpuWindows win;
...
static inline __attribute__((always_inline)) void PpuSetMode7PerspectiveCorrection(Ppu *ppu, int low, int high) {
  ppu->mode7PerspectiveLow = low ? 1.0f / low : 0.0f;
  ppu->mode7PerspectiveHigh = 1.0f / high;
}

static inline __attribute__((always_inline)) void PpuSetExtraSideSpace(Ppu *ppu, int left, int right, int bottom) {
...
static inline __attribute__((always_inline)) float FloatInterpolate(float x, float xmin, float xmax, float ymin, float ymax) {
...
static inline __attribute__((always_inline)) void PpuDrawMode7Upsampled(Ppu *ppu, uint y) {
  // expand 13-bit values to signed values
...
static inline __attribute__((always_inline)) void ClearBackdrop(PpuPixelPrioBufs *buf) {
  for (size_t i = 0; i != countof(buf->data); i++)
    buf->data[i] = 0x0500;
}
...
static inline __attribute__((always_inline)) int ppu_getPixelForBgLayer(Ppu *ppu, int x, int y, int layer, bool priority) {
...
static inline __attribute__((always_inline)) int ppu_getPixelForMode7(Ppu* ppu, int x, int layer, bool priority) {
...
static inline __attribute__((always_inline)) int ppu_getPixel(Ppu *ppu, int x, int y, bool sub, int *r, int *g, int *b) {
...
static inline __attribute__((always_inline)) void ppu_handlePixel(Ppu* ppu, int x, int y) {
  int r = 0, r2 = 0;
  int g = 0, g2 = 0;
  int b = 0, b2 = 0;
  if (!ppu->forcedBlank) {
...
static inline __attribute__((always_inline)) bool ppu_evaluateSprites(Ppu* ppu, int line) {
...
static inline __attribute__((always_inline)) void ppu_calculateMode7Starts(Ppu* ppu, int y) {
  // expand 13-bit values to signed values
  int hScroll = ((int16_t) (ppu->m7matrix[6] << 3)) >> 3;
  int vScroll = ((int16_t) (ppu->m7matrix[7] << 3)) >> 3;
  int xCenter = ((int16_t) (ppu->m7matrix[4] << 3)) >> 3;
...
static inline __attribute__((always_inline)) void ppu_runLine(Ppu* ppu, int line) {
  if(line != 0) {
...
static inline __attribute__((always_inline))  uint8_t ppu_read(Ppu* ppu, uint8_t adr) {
  switch (adr) {
  case 0x34:
  case 0x35:
  case 0x36: {
...
static inline __attribute__((always_inline)) void ppu_write(Ppu* ppu, uint8_t adr, uint8_t val) {
  switch(adr) {
    case 0x00: {  // INIDISP
